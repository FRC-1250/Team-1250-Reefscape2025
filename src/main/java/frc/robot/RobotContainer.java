...
import frc.robot.commands.AlgaeEndEffector.IntakeAlgaeWithDelayedStop;
...
public class RobotContainer {
        ...
        private final Trigger isBlue = new Trigger(() -> drivetrain.getOperatorForwardDirection().getDegrees() == 0); // Check
                                                                                                                      // if
                                                                                                                      // operator
                                                                                                                      // direction
                                                                                                                      // is
        ...
        private final Trigger isAtBarge = new Trigger(
                () -> elevator.isAtPosition(ElevatorPosition.BARGE));
        ...
        private void configureBindings() {
               ...
               /* joystick.rightTrigger()
                ...
                joystick.rightTrigger()
                        .and(hasAlgae)
                        .and(isAtHome)
                        .onTrue(new ReleaseAlgae(algaeEndEffector, IntakeVelocity.YEET));
                ...
                joystick.b()
                                ...
                                .onTrue(controlFactory
                                                .homeEndEffectorAndSetElevatorPosition(
                                                                (Elevator.ElevatorPosition.HOME)));
                joystick.b()
                ...
                joystick.rightBumper()
                        .and(isAtHome)
                        .onTrue(new SetWristPosition(algaeEndEffector, WristPosition.FLOOR)
                        .andThen(new IntakeAlgae(algaeEndEffector, IntakeVelocity.INTAKE)));

                joystick.rightBumper()
                        .and(isAtHighAlgaePosition)
                        .onTrue(new SetWristPosition(algaeEndEffector, WristPosition.REEF)
                        .andThen(new IntakeAlgae(algaeEndEffector, IntakeVelocity.INTAKE)));
                ...
                if (driveEnabled) {
                        drivetrain.setDefaultCommand(
                                        drivetrain.applyRequest(() -> drive
                                                        .withVelocityX(yLimiter
                                                                        .calculate(-joystick.getLeftY() * MaxSpeed))
                                                        .withVelocityY(xLimiter
                                                                        .calculate(-joystick.getLeftX() * MaxSpeed))
                        ...
                        joystick.x().whileTrue(drivetrain.applyRequest(
                                        () -> driveFacingAngle
                                                        ...
                                                        .withVelocityY(xLimiter
                                                                        .calculate(-joystick.getLeftX() * MaxSpeed))
                                                        .withTargetDirection(Rotation2d.fromDegrees(drivetrain
                                                                        .getOperatorForwardDirection().getDegrees())))
                                        .withName("Forward and strafe"));

                        // Red
                        joystick.a().and(isId6).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(6), 0));
                        joystick.a().and(isId7).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(7), 0));
                        joystick.a().and(isId8).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(8), 0));
                        joystick.a().and(isId9).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(9), 0));
                        joystick.a().and(isId10).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(10), 0));
                        joystick.a().and(isId11).and(isBlue).negate().whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(11), 0));

                        // Blue
                        joystick.a().and(isId17).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(17), 0));
                        joystick.a().and(isId18).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(18), 0));
                        joystick.a().and(isId19).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(19), 0));
                        joystick.a().and(isId20).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(20), 0));
                        joystick.a().and(isId21).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(21), 0));
                        joystick.a().and(isId22).and(isBlue).whileTrue(controlFactory
                                        .cmdPathfindToPose(ReefScoringMap.getReefPoseFromLimelightID(22), 0));

                        joystick.start().onTrue(drivetrain.runOnce(() -> drivetrain.seedFieldCentric())
                                        .withName("Reseed drive"));
                }

                if (automationEnabled) {
                        isTeleop.and(hasCoral.negate()).onTrue(Commands.waitSeconds(0.2)
                                        .andThen(endEffector.cmdSetHeadRotation(EndEffector.HeadPosition.CENTER)));
                        ...
                        isTeleop.and(hasCoral.negate()).and(isAtHome)
                                        .whileTrue(endEffector.cmdSetCoralDutyCycleOut(.05));
                ...
                if (devController) {
                        ...
                        devJoystick.axisLessThan(0, -0.75)
                                        .whileTrue(endEffector
                                                        .cmdSetHeadRotation(EndEffector.HeadPosition.CENTER_LEFT));
                        ...
                        devJoystick.axisGreaterThan(5, 0.75)
                                        .onTrue(endEffector.cmdSetAlgaeIntakePostion(
                                                        EndEffector.AlgaeServoPosition.DEPLOYED));
                        devJoystick.axisLessThan(5, -0.75)
                                        .onTrue(endEffector
                                                        .cmdSetAlgaeIntakePostion(EndEffector.AlgaeServoPosition.HOME));
                        devJoystick.R3().onTrue(
                                        endEffector.cmdSetAlgaeIntakePostion(EndEffector.AlgaeServoPosition.MIDDLE));
        ...
        private void configureSmartDashboardBindings() {
                ...
                SmartDashboard.putData(
                                endEffector.cmdSetHeadRotation(HeadPosition.LOGICAL_CENTER).withName("Head, center"));
                ...
                SmartDashboard.putData(
                                endEffector.cmdSetAlgaeIntakePostion(AlgaeServoPosition.MIDDLE)
                                                .withName(" Intake, middle"));
        ...
        private void addPathAuto(String name, String pathName) {
                try {
                        ...
                        DataLogManager.log(
                                        String.format("GatorBot: Not able to build auto routines! %s", e.getMessage()));
                }
        }
        ...
        private void configureNamedCommands() {
                ...
                NamedCommands.registerCommand("RotateHeadLeft",
                                endEffector.cmdSetHeadRotation(HeadPosition.CENTER_LEFT));
                NamedCommands.registerCommand("RotateHeadRight",
                                endEffector.cmdSetHeadRotation(HeadPosition.CENTER_RIGHT));
                ...
                NamedCommands.registerCommand("AutoIndexer",
                                endEffector.cmdAutoIndexer().andThen(endEffector.cmdAddCoralRotations(5)));
                NamedCommands.registerCommand("HighAlgeaPrep",
                                elevator.cmdSetPosition(ElevatorPosition.HIGH_ALGAE_PREP));
                // NamedCommands.registerCommand("LowAlgeaPrep",
                // elevator.cmdSetPosition(Position.LOW_ALGAE_PREP));
                ...
                NamedCommands.registerCommand(
                        "DealgeaHighGoHome", 
                        new SetElevatorPosition(elevator, ElevatorPosition.HIGH_ALGAE).
                                andThen( new SetWristPosition(algaeEndEffector, WristPosition.REEF))
                                .andThen(new IntakeAlgaeWithDelayedStop(algaeEndEffector))
